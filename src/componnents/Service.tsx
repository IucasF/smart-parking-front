...
async function getEstacionamentos(): Promise<IEstacionamento[]> {
    const response = await fetch('https://localhost:3000/estacionamentos');
    if (!response.ok) {
...
async function getEstacionamentoByID(id: string | undefined): Promise<IEstacionamento> {
    const response = await fetch(`https://localhost:3000/estacionamentos/${id}`);
    if (!response.ok) {
...
async function getVagasByID(id: string | undefined): Promise<IVagas> {
    const response = await fetch(`https://localhost:3000/vagas/${id}`);
    if (!response.ok) {
